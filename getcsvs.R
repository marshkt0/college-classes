...
write.csv( read_csv(url,header=T), "csvs/advent02.csv", quote=F, row.names=F ) ;
...
for(i in 3:9){
  ...
  write.csv( read_csv(url,header=T),fname, quote=F, row.names=F ) ;
...
write.csv( read_csv(url,header=T), "csvs/easter17.csv", quote=F, row.names=F ) ;
